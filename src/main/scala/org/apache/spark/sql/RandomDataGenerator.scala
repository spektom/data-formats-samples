...
object RandomDataGenerator {
  ...
  def randomDataFrame(spark: SparkSession,
                      numFields: Int = 150,
                      numRows: Int = 300,
                      flatSchema: Boolean = false): DataFrame = {
    val types = Array(
      BooleanType, ByteType, FloatType, DoubleType, IntegerType, LongType, ShortType, StringType,
      DecimalType.ShortDecimal, DecimalType.IntDecimal, DecimalType.ByteDecimal, DecimalType.FloatDecimal, DecimalType.LongDecimal,
      new DecimalType(5, 2), new DecimalType(12, 2), new DecimalType(30, 10))
    ...
    val rows = (1 to numRows).map(_ => RandomDataGenerator.randomRow(random, schema))
